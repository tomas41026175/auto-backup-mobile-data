...
const btnVariant: Record<ButtonVariant, string> = {
  primary:   'bg-[--color-primary] text-white hover:bg-[--color-primary-hover] shadow-[--shadow-glow] hover:shadow-[0_0_28px_var(--color-primary-glow)]',
  secondary: 'border border-[--color-border-strong] bg-[--color-bg-raised] text-[--color-text] hover:bg-[--color-bg-overlay] hover:border-[--color-border-strong]',
  ghost:     'text-[--color-text-secondary] hover:bg-[--color-bg-raised] hover:text-[--color-text]',
  danger:    'bg-[--color-error-subtle] border border-[--color-error]/30 text-[--color-error] hover:bg-[--color-error]/20',
}
...
export function Button({
  ...
  isLoading, icon, disabled, children, className, ...props
}: ButtonProps) {
  return (
    <button
...
export function Badge({ variant = 'default', dot, children, className }: BadgeProps) {
  return (
    <span className={cn(
      ...
      {dot && <span className={cn('h-1.5 w-1.5 rounded-full bg-current', variant === 'success' && 'animate-pulse')} />}
      {children}
    </span>
  )
}
...
export function Progress({ value, className, animated }: ProgressProps) {
  return (
    <div className={cn('h-1.5 w-full overflow-hidden rounded-full bg-[--color-bg-overlay]', className)}>
      <div
        className={cn(
          'h-full rounded-full bg-[--color-primary] transition-all duration-500',
          animated && 'relative after:absolute after:inset-0 after:bg-gradient-to-r after:from-transparent after:via-white/20 after:to-transparent after:animate-shimmer',
        )}
